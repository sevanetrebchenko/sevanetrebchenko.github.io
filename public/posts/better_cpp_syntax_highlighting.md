...
```
This function simply registers an `Annotation` instance with the given name and location.
...
With the logic to insert annotations into the source code implemented, the next task is to implement a mechanism that will allow us to retrieve a subset of tokens that fall within a `SourceRange`.
Why?
...
It does, however, include a way to retrieve the range of the node - spanning from a start to an end `SourceLocation` - which helps us narrow down our search.
...
The `Annotator` and `Tokenizer` are member variables of the `ASTConsumer` class.
```cpp
```
