...
- Function exception specification type (`throw(...)/nothrow`, `noexcept`, etc.)
...
For this project, we will be deriving from the `ASTFrontendAction` interface, which provides a way to parse and traverse the AST for a given translation unit (which is convenient, as it means I don't need to go through the trouble of [writing one from scratch]()).
This will allow us to perform a preorder traversal of the AST and extract information about the nodes we care about.

An instance of a `ASTFrontendAction` needs to define 

## Prerequisites

For this project, we configure a `RecursiveASTVisitor` in the following way:
There is a lot we can do here, but the majority of the heavy lifting is done by the `Visitor`.
