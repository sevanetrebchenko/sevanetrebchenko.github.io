...
The traversal of the AST is kicked off in `HandleTranslationUnit`, starting with the root `TranslationUnitDecl` node:
```cpp line-numbers:{enabled}
...
#### Optional: Configuring Traversal
...
Finally, we invoke the tool using `runToolOnCodeWithArgs`, specifying the `ASTFrontendAction`, source code, and any [command line arguments](https://clang.llvm.org/docs/ClangCommandLineReference.html):
```cpp line-numbers:{enabled}
#include "action.hpp"
